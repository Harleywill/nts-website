...
import { FolderOpen, FileText, Mail, Users, TrendingUp, ArrowRight } from 'lucide-react';

export default function AdminDashboard() {
  const [stats, setStats] = useState({ totalProjects: 0, featuredProjects: 0, newEnquiries: 0, teamMembers: 0, totalTestimonials: 0, totalNews: 0 });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const enquiriesData = await fetch('/api/contact-submissions').then(r => r.json()).catch(() => []);
        const users = await fetch('/api/users').then(r => r.json()).catch(() => []);
        const testimonials = await fetch('/api/testimonials').then(r => r.json()).catch(() => []);
        const news = await fetch('/api/news').then(r => r.json()).catch(() => []);

        const projectsArr = Array.isArray(projects) ? projects : [];
        const enquiriesArr = Array.isArray(enquiriesData) ? enquiriesData : [];
        ...
        setStats({
          ...
          featuredProjects: projectsArr.filter(p => p.featured).length,
          newEnquiries: enquiriesArr.filter(e => !e.read).length,
          teamMembers: usersArr.length,
          totalTestimonials: testimonialsArr.length,
          totalNews: newsArr.length,
        });

        setRecentProjects(projectsArr.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 4));
        setFeaturedProjects(projectsArr.filter(p => p.featured).slice(0, 3));
        setEnquiries(enquiriesArr.filter(e => !e.read).slice(0, 4));
      } catch (error) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '20px', alignItems: 'start', marginBottom: '20px' }}>
        ...
        <SectionCard title="Latest enquiries" action={<LinkButton href="/admin/contact-submissions">Inbox <ArrowRight size={14} /></LinkButton>}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {enquiries.map((e, i) => (
              <button key={e.id} onClick={() => window.location.href = '/admin/contact-submissions'} style={{ display: 'flex', gap: '12px', padding: '13px 0', borderTop: i ? '1px solid var(--border)' : 'none', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%', alignItems: 'flex-start' }}>
                <div style={{ width: '8px', marginTop: '6px', flex: 'none' }}>
                  {!e.read && <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--green-600)', display: 'block' }}></span>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '14px', color: 'var(--navy-800)' }}>{e.name}</div>
            ...
            {enquiries.length === 0 && <p style={{ fontFamily: 'var(--font-body)', fontSize: '14px', color: 'var(--slate-500)', margin: 0 }}>No enquiries</p>}
          </div>
        </SectionCard>
      </div>
